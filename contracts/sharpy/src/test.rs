...
#[cfg(test)]
mod test_event_taxonomy_updated {
    use soroban_sdk::{testutils::Address as _, Address, Env, Vec};
    use crate::SharpyContractClient;
    ...
    #[test]
    fn test_discount_mutation_is_audited() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let id = mk(&env, &client, &creator);
        client.set_discount(&creator, &id, &250u32);
        assert_eq!(client.get_discount(&id).unwrap().discount_bps, 250u32);
        let log = client.get_audit_log(&id);
        assert!(log.iter().any(|e| e.action == soroban_sdk::symbol_short!("disc")));
    }

    #[test]
    fn test_whitelist_mutations_update_state() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let payer_a = Address::generate(&env);
        let payer_b = Address::generate(&env);
...
mod test_fee_estimate_bounds {
    ...
    fn mk(env: &Env, client: &SharpyContractClient<'_>, creator: &Address, amounts: &[i128]) -> u64 {
        let opts = crate::types::InvoiceOptions {
            escrow_enabled: false,
            escrow_release_delay: None,
            split_rules: Vec::new(env),
        ...
        let mut toks = Vec::new(env);
        for a in amounts.iter() {
            recipients.push_back(Address::generate(env));
            amts.push_back(*a);
            toks.push_back(Address::generate(env));
        }
        let dl = env.ledger().timestamp() + 86400;
        client.create_invoice(creator, &recipients, &amts, &toks, &dl, &opts)
    }

    #[test]
    fn test_fee_estimate_zero_when_unset() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let id = mk(&env, &client, &creator, &[10_000i128]);
        assert_eq!(client.preview_fee_for_invoice(&id), 0i128);
    }

    #[test]
    fn test_fee_estimate_matches_amount_preview_on_total() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let collector = Address::generate(&env);
        client.set_protocol_fee(&250u32, &collector);
        let id = mk(&env, &client, &creator, &[10_000i128]);
        assert_eq!(client.preview_fee_for_invoice(&id), 250i128);
        assert_eq!(client.preview_fee_for_invoice(&id), client.preview_fee(&10_000i128));
    }

    #[test]
    fn test_fee_estimate_zero_bps_stays_zero() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let collector = Address::generate(&env);
        client.set_protocol_fee(&0u32, &collector);
...
mod test_fee_estimate_totals {
    ...
    #[test]
    fn test_fee_estimate_sums_multi_recipient_total() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let collector = Address::generate(&env);
        client.set_protocol_fee(&100u32, &collector);
        let id = mk(&env, &client, &creator, &[6000i128, 4000i128]);
        assert_eq!(client.preview_fee_for_invoice(&id), 100i128);
    }

    #[test]
    fn test_fee_estimate_is_pure_view() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let collector = Address::generate(&env);
        client.set_protocol_fee(&500u32, &collector);
        let id = mk(&env, &client, &creator, &[10_000i128]);
        assert_eq!(client.preview_fee_for_invoice(&id), 500i128);
        // Estimate must not mutate stored config
        assert_eq!(client.get_protocol_fee().unwrap().fee_bps, 500u32);
        assert_eq!(client.get_invoice(&id).funded, 0i128);
    }
}
...
mod test_route_chain_b {
    ...
    #[test]
    fn test_chain_three_hops_resolves() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let a = mk(&env, &client, &creator);
        let b = mk(&env, &client, &creator);
    ...
    #[test]
    fn test_chain_depth_zero_returns_start() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let a = mk(&env, &client, &creator);
        let b = mk(&env, &client, &creator);
        client.set_route(&creator, &a, &b);
        assert_eq!(client.resolve_route_chain(&a, &0u32), a);
...
mod test_tranche_invariant_c {
    ...
    fn test_tranche_isolated_per_invoice() {
        ...
        let a = mk(&env, &client, &creator);
        let b = mk(&env, &client, &creator);
        client.release_tranche(&creator, &a, &2500u32);
        assert_eq!(client.get_released_bps(&a), 2500u32);
        assert_eq!(client.get_tranche_remaining_bps(&a), 7500u32);
        assert_eq!(client.get_released_bps(&b), 0u32);
        assert_eq!(client.get_tranche_remaining_bps(&b), 10_000u32);
    }
    #[test]
    #[should_panic(expected = "only creator can release tranches")]
    fn test_non_creator_tranche_panics() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let other = Address::generate(&env);
        let id = mk(&env, &client, &creator);
...
mod test_paged_total_c {
    ...
    #[test]
    fn test_paging_walk_reconstructs_list() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        for _ in 0..5 {
            mk(&env, &client, &creator);
        }
        let total = client.get_creator_invoice_total(&creator);
        assert_eq!(total, 5u32);
        let p0 = client.get_creator_invoices_paged(&creator, &2u32, &0u32);
        let p1 = client.get_creator_invoices_paged(&creator, &2u32, &2u32);
        let p2 = client.get_creator_invoices_paged(&creator, &2u32, &4u32);
        assert_eq!(p0.len() + p1.len() + p2.len(), total);
        let full = client.get_invoices_by_creator(&creator);
        assert_eq!(p0.get(0).unwrap(), full.get(0).unwrap());
        assert_eq!(p2.get(0).unwrap(), full.get(4).unwrap());
    }
    #[test]
    fn test_truncation_returns_short_page() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        mk(&env, &client, &creator);
        mk(&env, &client, &creator);
...
mod test_ttl_terminal_b {
    ...
    #[test]
    fn test_released_terminal_hint_zero() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let payer = Address::generate(&env);
        let admin = Address::generate(&env);
        ...
        let opts = crate::types::InvoiceOptions { escrow_enabled: false, escrow_release_delay: None, split_rules: Vec::new(&env), auto_resolve_rules: Vec::new(&env), arbitrator: None };
        let r = Address::generate(&env);
        let id = client.create_invoice(&creator, &Vec::from_array(&env, [r]), &Vec::from_array(&env, [1000i128]), &Vec::from_array(&env, [tok]), &dl, &opts);
        client.pay(&payer, &id, &1000i128);
    ...
    #[test]
    fn test_cancelled_terminal_hint_zero() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let r = Address::generate(&env);
        let tok = Address::generate(&env);
...
mod test_ttl_terminal_c {
    ...
    #[test]
    fn test_refunded_terminal_hint_zero() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let payer = Address::generate(&env);
        let admin = Address::generate(&env);
        let tok = env.register_stellar_asset_contract(admin);
        token::StellarAssetClient::new(&env, &tok).mint(&payer, &2000i128);
        let now = env.ledger().timestamp();
        let opts = crate::types::InvoiceOptions { escrow_enabled: false, escrow_release_delay: None, split_rules: Vec::new(&env), auto_resolve_rules: Vec::new(&env), arbitrator: None };
        let r = Address::generate(&env);
        let id = client.create_invoice(&creator, &Vec::from_array(&env, [r]), &Vec::from_array(&env, [1000i128]), &Vec::from_array(&env, [tok]), &(now + 10), &opts);
        client.pay(&payer, &id, &400i128);
        env.ledger().set_timestamp(now + 11);
        client.refund(&id);
        assert_eq!(client.get_invoice(&id).status, InvoiceStatus::Refunded);
    ...
    fn test_extend_revives_hint() {
        ...
        let now = env.ledger().timestamp();
        let opts = crate::types::InvoiceOptions { escrow_enabled: false, escrow_release_delay: None, split_rules: Vec::new(&env), auto_resolve_rules: Vec::new(&env), arbitrator: None };
        let id = client.create_invoice(&creator, &Vec::from_array(&env, [r]), &Vec::from_array(&env, [100i128]), &Vec::from_array(&env, [tok]), &(now + 100), &opts);
        client.extend_deadline(&creator, &id, &(now + 1000));
        assert_eq!(client.get_ttl_hint(&id), 1000u64);
        assert!(!client.is_invoice_expired(&id));
    }
}
...
mod test_invariant_a {
    ...
    #[test]
    fn test_funded_plus_remaining_equals_total() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let payer = Address::generate(&env);
        let admin = Address::generate(&env);
        ...
        let opts = crate::types::InvoiceOptions { escrow_enabled: false, escrow_release_delay: None, split_rules: Vec::new(&env), auto_resolve_rules: Vec::new(&env), arbitrator: None };
        let id = client.create_invoice(&creator, &Vec::from_array(&env, [r]), &Vec::from_array(&env, [1000i128]), &Vec::from_array(&env, [tok]), &dl, &opts);
        assert_eq!(client.get_funding_remaining(&id), 1000i128);
        client.pay(&payer, &id, &400i128);
        assert_eq!(client.get_funding_remaining(&id), 600i128);
        assert_eq!(client.get_invoice(&id).funded + client.get_funding_remaining(&id), 1000i128);
    }
    #[test]
    #[should_panic(expected = "payment exceeds remaining balance")]
    fn test_overpay_panics() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let payer = Address::generate(&env);
        let admin = Address::generate(&env);
...
mod test_invariant_c {
    ...
    fn test_double_refund_panics() {
        ...
        let now = env.ledger().timestamp();
        let opts = crate::types::InvoiceOptions { escrow_enabled: false, escrow_release_delay: None, split_rules: Vec::new(&env), auto_resolve_rules: Vec::new(&env), arbitrator: None };
        let id = client.create_invoice(&creator, &Vec::from_array(&env, [r]), &Vec::from_array(&env, [100i128]), &Vec::from_array(&env, [tok]), &(now + 10), &opts);
        env.ledger().set_timestamp(now + 11);
        client.refund(&id);
        client.refund(&id);
    }
    #[test]
    fn test_sequential_pays_respect_remaining() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let payer = Address::generate(&env);
        let admin = Address::generate(&env);
        ...
        client.pay(&payer, &id, &600i128);
        client.pay(&payer, &id, &400i128);
        assert_eq!(client.get_funding_remaining(&id), 0i128);
        assert!(client.is_invoice_terminal(&id));
    }
}

#[cfg(test)]
mod test_release_proof_a {
    use soroban_sdk::{testutils::Address as _, Address, Env, Vec};
    use crate::SharpyContractClient;
    fn setup() -> (Env, SharpyContractClient<'static>) {
        let env = Env::default();
    ...
    #[test]
    fn test_storage_keys_present_via_getters() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let r = Address::generate(&env);
        let tok = Address::generate(&env);
        ...
        let opts = crate::types::InvoiceOptions { escrow_enabled: false, escrow_release_delay: None, split_rules: Vec::new(&env), auto_resolve_rules: Vec::new(&env), arbitrator: None };
        let id = client.create_invoice(&creator, &Vec::from_array(&env, [r]), &Vec::from_array(&env, [100i128]), &Vec::from_array(&env, [tok]), &dl, &opts);
        assert!(client.get_invoice(&id).version == 1u32);
        assert_eq!(client.get_invoice_count(), 1u64);
        assert_eq!(client.get_invoice_version(&id), 1u32);
        assert_eq!(client.get_contract_version(), 1u32);
        assert_eq!(client.get_creator_invoice_total(&creator), 1u32);
    ...
    #[test]
    fn test_new_views_callable_release_proof() {
        let (env, client) = setup();
        let creator = Address::generate(&env);
        let r = Address::generate(&env);
        let tok = Address::generate(&env);
        ...
        let id = client.create_invoice(&creator, &Vec::from_array(&env, [r]), &Vec::from_array(&env, [100i128]), &Vec::from_array(&env, [tok]), &dl, &opts);
        assert_eq!(client.get_fee_bps(), 0u32);
        assert_eq!(client.get_funding_remaining(&id), 100i128);
        assert_eq!(client.get_tranche_remaining_bps(&id), 10_000u32);
        assert_eq!(client.get_audit_count(&id), 0u32);
        assert!(!client.is_invoice_terminal(&id));
    }
}
